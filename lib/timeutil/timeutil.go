...
// TimestampSToTime turn a uint64 timestamp in millisecond to a golang time object
func TimestampSToTime(s uint64) time.Time {
	return time.Unix(0, int64(s)*int64(time.Second))
}
...
// TimeToTimestampS turn a golang time object into uint64 timestamp in millisecond
func TimeToTimestampS(t time.Time) uint64 {
